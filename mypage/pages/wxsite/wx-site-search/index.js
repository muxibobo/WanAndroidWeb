...
function initData() {
    ...
    wxSiteId = urlParams.get('wxSiteId');
    if (wxSiteId) {
        // 此处已成功获取到wxSiteId的值
        console.log('wxSiteId:', wxSiteId);
        // 进一步处理wxSiteId...
    } else {
        console.error('未能从URL中获取到wxSiteId');
        return;
    }
    initTemplateFun();
    requestAritcle();
...
function requestAritcle() {
    ...
    fetch(`${base_url}/proxyApi/wxarticle/list/${wxSiteId}/${pageIndex}/json?k=${wxSearchStr}&page_size=${pageSize}`, {
        method: 'GET',
        headers: {
